...
Higher level control components used in forms: `<TextInput>`, `<NumberInput>`...

The `onChange` event handler is normalized, the callback function is called passing the **value** of the field instead of an `event` object.
